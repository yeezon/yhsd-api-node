 ...
 * @param callback 回调,接受 error 和该 token 的请求数两个参数
 * @type {null|Function}
 */
var getReqCountHandle = function (token, callback) {
 ...
 * @param callback 回调,接受一个 error 参数
 * @type {null|Function}
 */
var saveReqCountHandle = function (token, reqCount, callback) {
...
var reduceReqCountFn = function (token) {
  getReqCountHandle(token, function (err, reqCount) {
    if (err) {
      console.error(err);
      throw err;
    }
