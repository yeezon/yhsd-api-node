...
var request = function(options, params, callback) {
  if (requestCount == config.requestLimit) {
    setTimeout(request, config.requestStep, options, params, callback);
    return;
  }
  requestCount++;
  var req = config.http.request(options, function(res) {
    ...
    res.on('end', function() {
      ...
      try {
        ...
        setTimeout(fnReduceRequestCount, config.requestLimit * config.requestStep * eval(res.headers['x-yhsd-shop-api-call-limit']));
      }
      callback(null, data);
    });
